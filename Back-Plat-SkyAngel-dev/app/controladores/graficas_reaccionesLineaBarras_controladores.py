import pandas as pd
from app.mapper.graficas_reaccionesLineaBarras_mapper import reaccionesBarrasLineaMapper

class funreaccionesLineaBarrasService():

    def __init__(self):
        self.reaccionesBarrasLinea = reaccionesBarrasLineaMapper()
    ...
    def obtenerDatos(self, jsonParametros):
        try:
            ...
            data = self.reaccionesBarrasLinea.selectIncidentes(anios, mes, entidad, municipio, categoria)
            ...
            result = [
                {
                    "linea_transportista": row["linea_transportista"],
                    "crimenes": row["conteo"]
                }
                for row in data
            ]
            ...
            serie = {
                ...
                'chartWidth': [700],
                'chartHeight': [1500],
                # 'yAxisDataKey': [item["linea_transportista"] for item in result],
                'yAxisDataKey': "linea_transportista",
                'yAxisColor': '#0D125B',
                'yAxisWidth': [500],
            ...
            return {
                "title": "Total de delitos por Línea de Transportista",
                "serie": serie,
                "data": result
            }, 200
